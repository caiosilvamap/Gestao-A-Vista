...
class JsonToTable extends React.Component<JsonToTableProps<ElementRow>, State> {
    ...
    renderTableRows() {
        ...
        let classes = "";
        const { data, showDetails, sinter, Type, Pagination } = this.props;
                    ...
                    <tr key={index} className="even:bg-meta-9/90 dark:even:bg-meta-2/25" >
                        ...
                        {showDetails ? (
                            <td className={classes}>
                                <Link to={`/burden/sinter/${sinter}/details/${data.elements.find(e => e.column === 'Id')?.value}`}>
                                    Detalhes
                                </Link>
                            </td>
                        ) : null}
    ...
    render() {
        ...
        if (data.length > 0) {
            ...
            if (noData) {
                return (
                    <>
                        <div className="my-2"> {this.renderDateTimerPicker()}
                        </div>
